...
class LanceClient:
    ...
    def download_files_to_local_based_on_filelist(self, date, products_id = ['VNP02IMG', 'VNP03IMG'], collection_id='5110', data_path='data/VNPL1', julian_day='182'):
        for i in range(len(products_id)):
            ...
            # hh_list_na = hh_list_eu #= hh_list_na
            # vv_list_na = vv_list_eu #= vv_list_na
            ...
            for vnp_file in vnp_list:
                ...
                if not ((h_coor in hh_list_na and v_coor in vv_list_na) 
                            or (h_coor in hh_list_eu and v_coor in vv_list_eu)
                            or (h_coor in [27] and v_coor in [6])
                        ):
                    continue
